        ...
        <div class="row">
            ...
            <div class="col-md-9">
        		<div class="table-part">
                    <div>
        			    <h4 class="d-flex justify-content-between">Brand management: <button class="btn btn-info" data-toggle="modal" data-target="#myModal" id="add_btns">Add</button></h4>
<div class="modal" id="myModal">                 
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <!-- Modal body -->
            <form id="addBrand" method="post" action="#" onsubmit="return addBrand()" enctype="multipart/form-data">
...
<div class="modal" id="editModal<?= $value['id']; ?>">                 
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <!-- Modal Header -->
            <div class="modal-header">
                <h4 class="modal-title p-0">Edit Brand</h4>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
        
            <!-- Modal body -->
            <form id="editBrand" method="post" action="#" onsubmit="return editBrand(this , <?= $value['id']; ?>)" >
                <div class="col-md-12 py-3">
                    <div>
                        <label>Title</label>
                    ...
                    <div class="mt-3 text-center">
                        <button type="submit"  id="update<?= $value['id']; ?>" class="btn btn-success">Update</button>
                                            ...
                                            <button class="btn btn-danger" id="delete_btns" onclick="deleteBrand(<?= $value['id'] ?>)">Delete</button>
                                        </div>
                                    </td>
                                </tr>
                               <?php  $i++; }  }?>
...
function addBrand() {
    ...
    $.ajax({
        url: '<?= base_url() ?>/Admin/Brand/addBrand',
        type: 'POST',
        cache:false,
        contentType: false,
        processData: false,
        data:new FormData($('#addBrand')[0]),
        dataType: 'json',
        beforeSend: function() {        
...
function editBrand(el , id) {
    $('.alert-danger').remove();
    $.ajax({
        url: '<?= base_url() ?>/Admin/Brand/editBrand',
        type: 'POST',
        cache:false,
        contentType: false,
        processData: false,
...
function deleteBrand(id) {
    ...
    {
        $.ajax({
            url: '<?= base_url() ?>/Admin/Brand/deleteBrand',
            type: 'POST',
            cache:false,
            data:{'id':id},
            dataType: 'json',
